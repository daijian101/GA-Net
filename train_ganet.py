...
def main():
    @torch.no_grad()
    def infer_3d_volume(input_data):
        input_data = input_data['data']
        _, h, w, _ = input_data.shape
        input_data = input_data.permute((0, 3, 1, 2)).reshape((-1, h, w))

        batch_size = cfg.val_batch_size if cfg.val_batch_size < input_data.shape[1] else input_data.shape[1]
    ...
    def train():
        ...
        training_image_dir = cfg.dataset.slice_sample_dir.image
        tissue_label = cfg.tissue_label
        region_label = cfg.region_label
        training_label_dict = {tissue_label: cfg.dataset.slice_sample_dir[tissue_label],
                               region_label: cfg.dataset.slice_sample_dir[region_label]}
        ...
        training_dataset = BalancedForegroundRegionDataset(data_indices=training_samples,
                                                           image_properties=dataset_properties['study_properties'],
        ...
        volume_image_dir = cfg.dataset.volume_sample_dir.image
        val_label_dict = {tissue_label: cfg.dataset.volume_sample_dir[tissue_label]}
        ...
        # Train
        loss_criterion = DiceCELoss(to_onehot_y=True, softmax=True)
    ...
    def test():
        load_weights(cfg.checkpoint, network)
        test_sample = dataset_properties['test_set']
        volume_image_dir = cfg.dataset.volume_sample_dir.image
        test_label_dict = {cfg.tissue_label: cfg.dataset.volume_sample_dir[cfg.tissue_label]}
        ...
        for batch in tqdm(test_data_loader):
            ...
            vis_log.add_scalar(f'test/{study}/dsc', dsc)

            if cfg.save_test_segmentation_map:
                im0_file = os.path.join(cfg.dataset.volume_sample_dir.im0_dir, study + '.IM0')
                save_file = os.path.join(log_dir, 'test_result', f'{study}_{cfg.tissue_label}.BIM')
